...
ax[1].margins(x=.35);fig.suptitle('Hardware-constrained original tubes vs proposed trade-off\nIndependent ideal-model benchmarks; averages of two sample banks',fontsize=12);fig.text(.5,.015,f"Trade-off: paths {means['original']['trajectory_completion']*48:.0f}/48 → {means['proposed']['trajectory_completion']*48:.0f}/48; mean of bank P95 tracking errors {means['original']['trajectory_p95_mm']:.3f} → {means['proposed']['trajectory_p95_mm']:.3f} mm.",ha='center',color='#9b4422',fontsize=10);fig.tight_layout(rect=(0,.055,1,1));fig.savefig(out/'tradeoff_summary.png',dpi=180);plt.close(fig)
