...
class VisPyCTRTipControlViewer(joint_viewer.VisPyCTRViewer):
    ...
    def __init__(self, *, mode="hardware", tubes="original", workspace_samples=12000) -> None:
        ...
        self.canvas.title = "CTR Proposed Trade-off — Joint and Tip Control"
        ...
        self.view.camera.center = (0.0, 0.0, 90.0)
        self.view.camera.distance = 430.0
    ...
    def calculate_target_plan(self) -> None:
        ...
        if attempt.plan is None:
            ...
            self.last_action = "No solution found within coupled limits; target may be infeasible or solver may have stalled"
            self.update_target_visual()
            self.update_status()
            return
...
Coupled stops enforced; other collisions unmodelled.

WORKSPACE: {self.workspace_sample_count:,} valid sampled states
Approximate ideal-model selected-endpoint envelope.
