...
def build_motion_route(
    ...
    *,
    ...
    if retracted_deployment_m is None:
        retracted = np.array(deployment_limits.lower_m) if deployment_limits else np.zeros(3, dtype=float)
    else:
    ...
    if deployment_limits is not None:
        deployment_limits.validate(plan.start_deployment_m)
        deployment_limits.validate(plan.goal_deployment_m)
        deployment_limits.validate(retracted)
    ...
    if strategy == DIRECT_STRATEGY:
        ...
        phases = (
            MotionPhase(
                label="RETRACT TO MINIMUM EXPOSURE" if deployment_limits is not None else "RETRACT BEHIND PLATE",
                start_deployment_m=plan.start_deployment_m.copy(),
                start_rotation_rad=plan.start_rotation_rad.copy(),
...
class TipTargetPlanner:
    ...
    def __init__(
        ...
        *,
        ...
        self.local_solver.deployment_limits.validate(workspace.deployment_m)
        self.restart_solver.deployment_limits.validate(workspace.deployment_m)
        if self.local_solver.deployment_limits != self.restart_solver.deployment_limits:
            raise ValueError("Planning solvers must use the same deployment limits")
