...
from actuator_diagram import ActuatorDiagram
...
class VisPyCTRViewer:
    def __init__(self, *, mode="hardware", tubes="original", workspace_samples=12000) -> None:
        ...
        self.joint_motion_mode = "independent"
        self.following_actuators = ()
        self.undo_stack = []
        self.waypoints = []
        ...
        self.canvas = scene.SceneCanvas(
            keys="interactive",
            title="Measured Hardware CTR — Original Tubes",
            size=(1200, 800),
            bgcolor="white",
            dpi=96.0,
            show=False,
        ...
        self.actuator_diagram = ActuatorDiagram(self)
    ...
    def move(self, amount_mm: float) -> None:
        self.following_actuators = ()
        if getattr(self, "joint_motion_mode", "independent") == "coordinated":
            old = self.deployment.copy()
            result = coordinated_move(self.limits, old, self.selected_tube, amount_mm)
            self.following_actuators = tuple(i for i in range(3) if i != self.selected_tube and abs(result[i]-old[i])>1e-10)
            self.deployment[:] = result
            self.robot_dirty = True
            actual = (result[self.selected_tube]-old[self.selected_tube])*1000
            self.last_action = "Coordinated move"
            if self.following_actuators:
                self.last_action += "; following: " + ", ".join(TUBE_NAMES[i] for i in self.following_actuators)
            if abs(actual-amount_mm)>1e-6:
                self.last_action += "; selected actuator end-stop limit"
            return
        low, high = self.translation_limits(self.selected_tube)
        old = self.deployment[self.selected_tube]
        requested = old + amount_mm / 1000.0
        new = float(np.clip(requested, low, high))
        reason = self.limits.movement_reason(self.deployment, self.selected_tube, requested)
        self.last_action = reason if reason else "Joint movement within measured limits"
    ...
    def pose_record(self, label: str) -> dict[str, float | str]:
        ...
        return {
            ...
            "hardware_profile_id": self.profile.identity,
            "joint_motion_mode": self.joint_motion_mode,
            "max_gap_mm": self.limits.max_gap_mm,
            **{f"{name}_carriage_mm": float(value) for name, value in zip(("inner","middle","outer"), self.profile.carriage_displacement_mm(self.deployment))},
    ...
    def update_robot(self) -> None:
        ...
        if hasattr(self, "actuator_diagram"): self.actuator_diagram.update()
...
F6: gap 108 / 108.5 mm | F7: original / optimised (resets state)
C: joint control = {self.joint_motion_mode.upper()}
...
Q             Quit"""
    ...
    def profile_key(self, key):
        if key == "c":
            self.joint_motion_mode = "coordinated" if self.joint_motion_mode == "independent" else "independent"
            self.following_actuators = ()
            self.last_action = self.joint_motion_mode.title() + " joint control selected; Cartesian IK unchanged"
            self.update_status()
            return True
        if key == "f6":
            self.switch_profile(mode="hardware-108.5" if self.profile.mode == "hardware" else "hardware")
            return True
        if key == "f7":
    ...
    def load_waypoints(self, path):
        ...
        poses = []
        for row in rows:
            if row.get("hardware_profile_id") != self.profile.identity:
                raise ValueError("CSV must identify this measured hardware profile; legacy or different-gap exports are not accepted")
            if float(row.get("max_gap_mm", "nan")) != self.limits.max_gap_mm:
                raise ValueError("CSV maximum gap differs from active hardware")
