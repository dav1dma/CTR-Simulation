...
class ConstrainedTipIK:
    ...
    def validate_deployment(self, deployment_m: np.ndarray) -> np.ndarray:
        deployment = self._vector(deployment_m, "deployment_m")
        # Cached workspace states are stored as float32, so exact boundary
        # values can round a few nanometres beyond a nominal tube length.
        tolerance = 1e-7
        if np.any(deployment < -tolerance) or np.any(
            deployment > self.total_lengths + tolerance
        ):
            raise ValueError("A tube deployment is outside its physical limits.")
        if not (
            deployment[2] <= deployment[1] + tolerance
            and deployment[1] <= deployment[0] + tolerance
        ):
            raise ValueError("Deployment must satisfy outer <= middle <= inner.")
        deployment = np.clip(deployment, 0.0, self.total_lengths)
        deployment[1] = max(deployment[1], deployment[2])
        deployment[0] = max(deployment[0], deployment[1])
        return deployment

    def encode_deployment(self, deployment_m: np.ndarray) -> np.ndarray:
        """Encode valid deployments as three independent fractions."""
        inner, middle, outer = self.validate_deployment(deployment_m)
        outer_fraction = outer / self.total_lengths[2]
        middle_span = self.total_lengths[1] - outer
        inner_span = self.total_lengths[0] - middle
        middle_fraction = 0.0 if middle_span <= 1e-12 else (middle - outer) / middle_span
        inner_fraction = 0.0 if inner_span <= 1e-12 else (inner - middle) / inner_span
        return np.clip(
            [inner_fraction, middle_fraction, outer_fraction], 0.0, 1.0
        )

    def decode_deployment(self, fractions: np.ndarray) -> np.ndarray:
        """Decode fractions into physically valid nested deployments."""
        values = self._vector(fractions, "deployment fractions")
        inner_fraction, middle_fraction, outer_fraction = np.clip(values, 0.0, 1.0)
        outer = outer_fraction * self.total_lengths[2]
        middle = outer + middle_fraction * (self.total_lengths[1] - outer)
        inner = middle + inner_fraction * (self.total_lengths[0] - middle)
        return np.asarray([inner, middle, outer], dtype=float)
