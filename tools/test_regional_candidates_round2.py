...
DOUT=Path('results/regional_comparison_20260910_round2');DEST=Path('output/regional_comparison_round2');DOUT.mkdir(parents=True,exist_ok=True);DEST.mkdir(parents=True,exist_ok=True)
...
def run():
 ...
 designs += [np.array([km,ko,47.,27.5]) for km,ko in [(19.3,14.04),(19.12,14.2),(20,13),(22,12),(25,10),(28.68,7.02),(17,16),(15,18),(12,21.06),(18,15),(20,14),(19.12,13.8)]]
 ...
 _,bank=h.bank(1048576,16902);p=forward(bank,BASE);ids=h.cells(p);allcells=np.unique(np.concatenate(list(regions.values())));order=np.argsort(ids,kind='stable');u,b,c=np.unique(ids[order],return_index=True,return_counts=True);sel=[]
